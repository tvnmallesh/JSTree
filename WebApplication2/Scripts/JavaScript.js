...
; (function () {
    ...
    var jsPlumbInstance = function (_defaults) {
        ...
        this.Defaults = {
            ...
            EndpointOverlays: [],
            Endpoints: [null, null],
            EndpointStyle: { fillStyle: "#5a6aa4" },
            ...
            Overlays: [],
            MaxConnections: 1,
            PaintStyle: { lineWidth: 1, strokeStyle: "#5a6aa4" },
        ...
        this.connectorClass = "_jsPlumb_connector connector";
...
; (function () {
    ...
    jsPlumb.AnchorManager = function (params) {
        ...
        this.newConnection = function (conn) {
            if ($($(conn.source).offsetParent()).attr("id") != "divRegAnalysis") {
            ...
            var lblOverlays = conn.getOverlays();
            var label = '';
            $.each(lblOverlays, function (idx, overlay) {
                if (overlay.type == "Label") {
                    $(overlay.canvas).attr('target', conn.targetId);
                }
            });
        ...
        this.connectionDetached = function (connInfo) {
            //alert(htmlBase);
            if ($($(connInfo.source).offsetParent()).attr("id") != "divRegAnalysis") {
                $('#generateTCasesButton').hide();
                $('#addResultnData').hide();
                $('#addCreateRules').hide();
                $('#saveNUpdateButton').show();
    ...
    jsPlumb.Endpoint = function (params) {
        ...
        this.addConnection = function (connection) {
            ...
            if (connection.sourceId.indexOf("decision") > -1) {
                var overlays = connection.getOverlays();
                if (overlays.length <2) {
                    connection.addOverlay(["Label", { label: "Enter condition here", id: "label" }], true);
                }
            }
        };
    ...
    var AbstractDOMOverlay = function (params) {
        ...
        var makeDiv = function () {
            ...
            $(div).on("blur", function () {
                changeOverlay(event, this);
            });

            $(div).on("dblclick", function () {
                //$($(this).next('._jsPlumb_overlay')).focus();
                //$('._jsPlumb_overlay').removeClass("clsfocus");
                //var overlays = document.getElementsByClassName("_jsPlumb_overlay");

                //for (var i = 0; i < overlays.length; i++) {
                //    if ($(overlays[i]).attr("id") != $(this).attr("id") && $(overlays[i]).attr("source") == $(this).attr("source") && $(overlays[i]).attr("target") == $(this).attr("target")) {
                //        $(overlays[i]).addClass("clsfocus _jsplumb_hover");
                //    }
                //}
            ...
            if (params.source)
                $(div).attr("source", params.source);
            else
                $(div).attr("source", params.component.sourceId);

            $(div).attr("target", params.target);
